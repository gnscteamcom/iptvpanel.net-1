...
function addServer {
	...
	if [ -f /opt/iptvpanel2/bin/ffprobe ]; then
		...
		wget -O /opt/iptvpanel2/bin/ffprobe hhttps://github.com/gnscteamcom/iptvpanel.net-1/raw/master/ffprobe  >> /dev/null 2>&1
		wget -O /opt/iptvpanel2/bin/ffmpeg https://github.com/gnscteamcom/iptvpanel.net-1/raw/master/ffmpeg  >> /dev/null 2>&1
...
function upgradeFiles {
	...
	if [ "$ISSTREAMER" = "1" ]; then
		...
		wget -O /tmp/iptvpanel.tgz https://github.com/gnscteamcom/iptvpanel.net-1/raw/master/iptvpanel2-streamer.tgz >> /dev/null 2>&1
		tar xzvf /tmp/iptvpanel.tgz -C /opt >> /dev/null 2>&1
		rm -rf /tmp/iptvpanel.tgz >> /dev/null 2>&1
		echo -e "${BGreen}\t\t\tDone.${Reset}"
	fi
...
function installCMSPackages {
	...
	wget -N -O /usr/share/GeoIP.dat.gz https://github.com/gnscteamcom/iptvpanel.net-1/raw/master/GeoIP.dat.gz >> /dev/null 2>&1
